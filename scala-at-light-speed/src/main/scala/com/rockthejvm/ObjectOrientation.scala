...
class ObjectOrientation extends App {
  // java equivalent => public static main method(String[] args)
  // class and instance
  class Animal {
    // define fields
    val age: Int = 0
  ...
  trait Philosopher {
    def ?!(thought: String): Unit // valid method method
  }
  ...
  aCroc.eat(aDog)
  aCroc eat aDog // infix notation = {object | method | argument}, only available for methods with ONE method
  ...
  try {
    ...
    val x: String = null
    x.length
  } catch { // catch(Exception e) {...} -> java
    case e: Exception => "some faulty error message"
  } finally {
    // execute some code no matter what
  }

  // generics -> can be used for abstract class and traits
